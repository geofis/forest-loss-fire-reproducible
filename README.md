...
Follow the sequence listed below by running the code chunks inside the `.Rmd` files, or knitting them.

1. `data-download-preparation.Rmd`. This step is included here for completeness reasons, but is not mandatory, since the outcomes generated by this script should be available for use in the `data` and `out` directories after downloading the Zenodo data repo `forest-loss-fire-reproducible-data-repo`.
...
6. Run the code chunks in the reproducible scripts, by following the sequence mentioned in [Script execution sequence](#Script_execution_sequence) section.
